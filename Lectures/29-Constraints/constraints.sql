...
INSERT INTO Product VALUES('A', 3008, 'printer');
INSERT INTO Product VALUES('A', 2101, 'laptop');
...
CREATE TABLE Product (
    ...
    CHECK ((type = 'pc' AND model >= 1000 AND model < 2000) OR
           (type = 'laptop' AND model >= 2000 AND model < 3000) OR
           (type = 'printer' AND model >= 3000 AND model < 4000))
);
...
CREATE TABLE PC (
    model INTEGER REFERENCES Product(model) DEFERRABLE INITIALLY DEFERRED,
    speed FLOAT,
    ram INTEGER,
    hd INTEGER,
    price DECIMAL(7,2) NOT NULL,
...
CREATE TABLE PC (
    model INTEGER REFERENCES Product(model) ON DELETE CASCADE ON UPDATE SET NULL,
    speed FLOAT,
    ram INTEGER,
    hd INTEGER,
    price DECIMAL(7,2) NOT NULL,
...
CREATE TABLE Laptop (
    model INTEGER REFERENCES Product(model) ON UPDATE CASCADE ON DELETE SET NULL,
    speed FLOAT,
...
CREATE TABLE Printer (
    ...
    type VARCHAR(30),
    price decimal(7,2) NOT NULL,
    FOREIGN KEY(model) REFERENCES Product(model) ON UPDATE NO ACTION ON DELETE SET NULL
);
